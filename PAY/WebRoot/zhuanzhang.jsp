		...
		<div style="width:100%;height:650px;">
			<div id="nav" >
				<ul id="mainul">
		            <li>
		                ...
		                <ul id='two'>
		                    <li id="zhuanzhang" style="background: url(img/combo.jpg) no-repeat 0px -116px;">转账到支付宝</li>
		                    ...
		                    <li id="chongzhi" style="background: url(img/combo.jpg) no-repeat 0px -175px;">手机充值</li>
						...
						$.post(
							...
							function(rtnData){
								if (parseInt(rtnData)>parseInt(money)) {
									...
									$.post(
										"BillServlet",
										"opr=add&tel="+tel+"&money="+money+"&typeid=4&description="+$("#description").val()+"&friendsId="+friendsId+"&stateid=6",
										function(rtnData){
			...
			function chongzhi(){
				...
				if (tel==""||money=="") {
					...
					$.post(
						"UserServlet",
						"opr=huafei&tel="+$("#telnum").val()+"&money="+$("#moneynum").val()+"&inorout=out",
						function(rtnData){
							if (rtnData=="true") {
								alert('true');
							} else {
					...
					$("#ewm").attr("src","http://qr.topscan.com/api.php?text=http://47.99.191.102:8080/PAY/UserServlet?opr=huafei"+","+tel+","+money);
					$("#ewm").show();
				}
